...
class OrderSearchAPITest(TestCase):
    def setUp(self):
        ...
        self.order1 = Order.objects.create(date=date(2023, 9, 15))
        self.order2 = Order.objects.create(date=date(2023, 9, 20))

        self.item1_order1 = OrderItem.objects.create(order=self.order1, barcode="123456", price=100, quantity=2)
        self.item2_order1 = OrderItem.objects.create(order=self.order1, barcode="789012", price=50, quantity=1)

        self.item1_order2 = OrderItem.objects.create(order=self.order2, barcode="456789", price=75, quantity=3)
    ...
    def test_valid_orders_search(self):
        ...
        response = self.client.post('/orders_search/', data, format='json')
